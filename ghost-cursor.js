...
function initGhostCursor() {
    const host = document.getElementById('ghost-cursor-container');
    if (!host) return;
    ...
    const bloomStrength = 0.1;
    const bloomRadius = 1.0;
    const bloomThreshold = 0.025;
    const brightness = 1;
    const color = '#3b82f6'; // Match our accent color
    ...
    function resize() {
        ...
        material.uniforms.iScale.value = Math.max(0.5, Math.min(2.0, Math.max(cssW, cssH) / 600));
